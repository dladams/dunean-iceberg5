...
TF1 *langaufit(TH1 *his, Double_t *fitrange, Double_t *startvalues, Double_t *parlimitslo, Double_t *parlimitshi, Double_t *fitparams, Double_t *fiterrors, Double_t *ChiSqr, Int_t *NDF)
{
   ...
   int npar = 5;
   TF1 *ffit = new TF1(FunName,langaufun,fitrange[0],fitrange[1],npar);
...
TF1* langaus(TH1* ph, bool doDraw) {
  string myname = "langaus: ";
   // Fitting SNR histo
   printf("Fitting...\n");
   ...
   ph->GetXaxis()->SetRangeUser(15.0, 50.0);
   int bmax = ph->GetMaximumBin();
   ph->GetXaxis()->SetRange(0, 0);
   double xmax = ph->GetBinLowEdge(bmax);
   double ymax = ph->GetBinContent(bmax);
   fr[0]=0.75*xmax;
   fr[1]=1.3*xmax;
   cout << myname << "Fit range: (" << fr[0] << ", " << fr[1] << endl;
   ...
   Double_t chisqr;
   Int_t    ndf;
   TF1 *fitsnr = langaufit(ph,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
...
void langaus() {
   ...
   TH1F *ph = new TH1F("snr","Signal-to-noise",400,0,400);

   for (Int_t i=0; i<100; i++) ph->Fill(i,data[i]);
